...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="<?= base_url('admin/jadwal/create') ?>" class="bg-accent hover:bg-accent-hover text-white px-4 py-2 rounded-lg flex items-center gap-2 transition">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="p-4 border-b">
            <h3 class="font-semibold text-gray-800">Jadwal Terbaru</h3>
        </div>
            ...
            <div class="space-y-4">
                ...
                <div class="p-4 bg-gray-50 rounded-lg">
                    ...
                    <p class="text-sm text-gray-500 mb-2">
                        <i class="fas fa-user mr-1"></i> <?= $jadwal['pengajar_nama'] ?>
                    </p>
                    <p class="text-sm text-gray-500 mb-3">
                        <i class="fas fa-calendar mr-1"></i> <?= date('d M Y', strtotime($jadwal['tanggal'])) ?> • <?= $jadwal['waktu'] ?>
                    </p>
                    <div class="flex gap-2">
                        <a href="<?= base_url('admin/jadwal/edit/' . $jadwal['id']) ?>" class="text-blue-600 text-sm hover:underline">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <button onclick="confirmDelete('<?= base_url('admin/jadwal/delete/' . $jadwal['id']) ?>', 'Yakin ingin menghapus jadwal ini?')" class="text-red-600 text-sm hover:underline">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: '<?= base_url('admin/jadwal/events') ?>',
        ...
        eventClick: function(info) {
            ...
            var content = `
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="font-medium text-gray-800 mb-1">${info.event.title}</p>
                        <p class="text-sm text-gray-500 mb-2">
                            <i class="fas fa-user mr-1"></i> ${props.pengajar || '-'}
                        </p>
                        <p class="text-sm text-gray-500 mb-3">
            ...
            openEventModal('Detail Jadwal', content);
        }
    });
    calendar.render();
});
